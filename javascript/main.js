...
document.addEventListener("mousedown", function (event) {
  let isClickInsideTaskList =
    formList.contains(event.target) || formDetails.contains(event.target);
...
function addTask(taskValue, taskId) {
  ...
  checkbox.addEventListener("change", function () {
    const taskId = labelTask.getAttribute("task-id");
    const isCompleted = checkbox.checked ? 1 : 0;

    fetch("contains/updateTaskStatus.php", {
      method: "POST",
      headers: {
